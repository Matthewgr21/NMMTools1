...
TOOL_CATEGORIES = {
    ...
    'it_admin': {
        ...
        'tools': [
            ...
            {'id': 'check_winrm', 'name': 'Check Remote Management', 'description': 'Test if WinRM is enabled and accessible on target', 'admin_required': True},
            {'id': 'remote_reboot', 'name': 'Remote Reboot', 'description': 'Remotely restart a computer', 'admin_required': True},
            {'id': 'get_remote_info', 'name': 'Remote System Info', 'description': 'Get system info via WMI (no WinRM needed)', 'admin_required': True},
            {'id': 'list_remote_software', 'name': 'Remote Installed Software', 'description': 'List software on remote computer via WMI', 'admin_required': True},
            {'id': 'enable_winrm_bulk', 'name': 'Bulk Enable WinRM', 'description': 'Enable WinRM on multiple computers from a list', 'admin_required': True},
        ]
    }
}
...
class PowerShellExecutor:
    ...
    WMI_BASED_TOOLS = ['enable_winrm', 'check_winrm', 'remote_reboot', 'get_remote_info',
                       'list_remote_software', 'enable_winrm_bulk']

    @staticmethod
    def execute_command(command, job_id=None, target_host=None, tool_id=None):
    ...
    @staticmethod
    def execute_async(command, job_id, target_host=None, tool_id=None):
        """Execute PowerShell command asynchronously"""
        def run():
            PowerShellExecutor.execute_command(command, job_id, target_host, tool_id)
...
def api_run_tool():
    ...
    if async_mode:
        PowerShellExecutor.execute_async(command, job_id, target_host, tool_id)
        return jsonify({'success': True, 'job_id': job_id, 'status': 'started'})
    else:
        result = PowerShellExecutor.execute_command(command, job_id, target_host, tool_id)
...
def handle_run_tool(data):
    ...
    if tool_id in TOOL_COMMANDS:
        ...
        def execute_and_emit():
            result = PowerShellExecutor.execute_command(TOOL_COMMANDS[tool_id], job_id, target_host, tool_id)
