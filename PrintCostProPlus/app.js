/**
 * Print Cost Pro+ - 3D Print Cost Calculator with STL/3MF Analysis
 * Version 1.0
 */
...
let state = {
    ...
    currentCalc: {
        parts: [],
        consumables: [],
        modelData: null  // Store parsed model data
    }
};

// Model Parser instance
let modelParser = null;

// ============================================================================
// STORAGE KEYS
// ============================================================================
const STORAGE_KEYS = {
    PRINTERS: 'pcpp_printers',
    MATERIALS: 'pcpp_materials',
    PRODUCTS: 'pcpp_products',
    QUOTES: 'pcpp_quotes',
    SALES: 'pcpp_sales',
    CONSUMABLES: 'pcpp_consumables',
    SETTINGS: 'pcpp_settings'
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initNavigation();
    initCalculator();
    initFileUpload();  // NEW: Initialize file upload
    initPrinters();
    initMaterials();
    initProducts();
    initSettings();
...
// ============================================================================
// FILE UPLOAD HANDLING (NEW FOR PLUS VERSION)
// ============================================================================
function initFileUpload() {
    ...
    // Apply estimation
    applyBtn.addEventListener('click', applyEstimationToCalculator);

    // Update estimation when settings change
    ['infillPercent', 'wallCount', 'topBottomLayers', 'lineWidth', 'layerHeight', 'supportPercent'].forEach(id => {
        document.getElementById(id).addEventListener('change', updateEstimationPreview);
    });
}
...
async function handleFileUpload(file) {
    ...
    try {
        ...
        document.getElementById('fileName').textContent = 'Parsing...';

        // Parse the file
        const modelData = await modelParser.parseFile(file);
        state.currentCalc.modelData = modelData;

        // Display file info
        displayFileInfo(modelData);

        // Show estimation settings
        document.getElementById('estimationSettings').style.display = 'block';

        // Update estimation preview
...
function updateEstimationPreview() {
    ...
    try {
        const estimate = modelParser.estimateMaterialUsage(settings, density);
        const supportEstimate = modelParser.estimateSupportMaterial(supportPercent, density);

        // Store for apply button
        state.currentCalc.estimation = {
            materialGrams: estimate.weightGrams,
            supportGrams: supportEstimate.weightGrams,
            printTimeHours: estimate.estimatedPrintTimeHours
        };
    } catch (error) {
...
function applyEstimationToCalculator() {
    if (!state.currentCalc.estimation) {
        alert('No estimation available. Please upload a file first.');
        return;
    }

    const est = state.currentCalc.estimation;

    // Apply values to calculator
    document.getElementById('materialUsed').value = est.materialGrams.toFixed(1);
    document.getElementById('supportMaterial').value = est.supportGrams.toFixed(1);
    document.getElementById('printTime').value = est.printTimeHours.toFixed(2);

    // Update calculation
    updateCalculation();

    alert('Estimation applied! You can adjust the values manually if needed.');
}

function clearFileUpload() {
    document.getElementById('fileUploadZone').style.display = 'block';
    document.getElementById('fileInfo').style.display = 'none';
    document.getElementById('estimationSettings').style.display = 'none';
    document.getElementById('modelFileInput').value = '';
    state.currentCalc.modelData = null;
    state.currentCalc.estimation = null;
}
...
function initCalculator() {
    ...
    // Update estimation when material changes (for density)
    document.getElementById('calcMaterial').addEventListener('change', updateEstimationPreview);
...
function saveQuote() {
    ...
    const quote = {
        ...
        modelData: state.currentCalc.modelData ? { ...state.currentCalc.modelData } : null,
...
function loadQuoteToCalculator(quoteId) {
    ...
    // Restore model data if available (Pro+ only feature)
    if (quote.modelData) {
        state.currentCalc.modelData = { ...quote.modelData };
    }
...
// ============================================================================
// PRINTERS (Same as standard version)
// ============================================================================
...
function saveMaterial() {
    ...
    const material = { name, type, color, costPerKg, density };
...
function saveConsumableTemplate() {
    ...
    state.consumableTemplates.push({ id: 'cons_' + Date.now(), name, cost, uses });
...
function exportAllData() {
    ...
    a.href = url;
    a.download = `print-cost-pro-plus-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
